...
export const useAdminUsefulInformationStore = defineStore(
  "adminUsefulInformation",
  () => {
    ...
    const createOne = async (payload: zod.infer<typeof createFormSchema>) => {
      ...
      const { error } = await $authFetch("/useful-informations").post(formData);

      if (!error.value) router.push(localePath("/admin/useful-informations"));
    ...
    const fetchOne = async (id: string | string[]) => {
      ...
      const { data, error } = await $authFetch(`/useful-informations/${id}`)
        .get()
    ...
    const updateOne = async (
      ...
      const { error } = await $authFetch(`/useful-informations/${id}`).post(
        formData
      );
    ...
    const deleteOne = async (id: number) => {
      ...
      const { error } = await $authFetch(`/useful-informations/${id}`).delete();
