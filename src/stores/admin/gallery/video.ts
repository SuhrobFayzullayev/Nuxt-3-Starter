...
export const useAdminVideoGalleryStore = defineStore(
  "adminVideoGallery",
  () => {
    ...
    const createOne = async (payload: zod.infer<typeof createFormSchema>) => {
      ...
      const { error } = await $authFetch("/video-gallery").post(formData);

      if (!error.value) router.push(localePath("/admin/gallery/video"));
    ...
    const fetchAll = async (page: string | number = 1) => {
      ...
      const { data, error } = await $authFetch(`/video-gallery?page=${page}`)
        .get()
        .json<{
          data: {
            data: Response[];
    ...
    const fetchOne = async (id: string | string[]) => {
      ...
      const { data, error } = await $authFetch(`/video-gallery/${id}`)
        .get()
    ...
    const updateOne = async (
      ...
      const { error } = await $authFetch(`/video-gallery/${id}`).post(formData);
    ...
    const deleteOne = async (id: number) => {
      ...
      const { error } = await $authFetch(`/video-gallery/${id}`).delete();
