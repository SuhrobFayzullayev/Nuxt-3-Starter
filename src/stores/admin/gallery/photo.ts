...
export const useAdminPhotoGalleryStore = defineStore(
  "adminPhotoGallery",
  () => {
    ...
    const createOne = async (payload: zod.infer<typeof createFormSchema>) => {
      ...
      const { error } = await $authFetch("/photo-gallery").post(formData);

      if (!error.value) router.push(localePath("/admin/gallery/photo"));
    ...
    const fetchAll = async (page: string | number = 1) => {
      ...
      const { data, error } = await $authFetch(`/photo-gallery?page=${page}`)
        .get()
        .json<{
          data: {
            data: Response[];
    ...
    const fetchOne = async (id: string | string[]) => {
      ...
      const { data, error } = await $authFetch(`/photo-gallery/${id}`)
        .get()
    ...
    const updateOne = async (
      ...
      const { error } = await $authFetch(`/photo-gallery/${id}`).post(formData);
    ...
    const deleteOne = async (id: number) => {
      ...
      const { error } = await $authFetch(`/photo-gallery/${id}`).delete();
