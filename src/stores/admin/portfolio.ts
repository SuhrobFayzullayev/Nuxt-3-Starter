...
export const useAdminPortfolioStore = defineStore("adminPortfolio", () => {
  ...
  const createOne = async (payload: zod.infer<typeof createFormSchema>) => {
    ...
    const { error } = await $authFetch("/portfolios").post(formData);

    if (!error.value) router.push(localePath("/admin/portfolio"));
  ...
  const fetchAll = async (page: string | number = 1) => {
    ...
    const { data, error } = await $authFetch(`/portfolios?page=${page}`)
      .get()
      .json<{
        data: {
          data: Response[];
  ...
  const fetchOne = async (id: string | string[]) => {
    ...
    const { data, error } = await $authFetch(`/portfolios/${id}`)
      .get()
  ...
  const updateOne = async (
    ...
    const { error } = await $authFetch(`/portfolios/${id}`).post(formData);
  ...
  const deleteOne = async (id: number) => {
    ...
    const { error } = await $authFetch(`/portfolios/${id}`).delete();
