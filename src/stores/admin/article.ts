...
export const formSchema = zod.object({
  ...
  date_added: zod.string().trim().nonempty("help.field_is_required"),
...
export const useAdminArticleStore = defineStore("adminArticle", () => {
  ...
  const createOne = async (payload: zod.infer<typeof createFormSchema>) => {
    ...
    const { error } = await $authFetch("/articles").post(formData);

    if (!error.value) router.push(localePath("/admin/articles"));
  ...
  const fetchAll = async (page: number | string = 1) => {
    ...
    const { data, error } = await $authFetch(`/articles?page=${page}`)
      .get()
      .json<{
        data: {
          data: Response[];
  ...
  const fetchOne = async (id: string | string[]) => {
    ...
    const { data, error } = await $authFetch(`/articles/${id}`)
      .get()
  ...
  const updateOne = async (
    ...
    const { error } = await $authFetch(`/articles/${id}`).post(formData);
  ...
  const deleteOne = async (id: number) => {
    ...
    const { error } = await $authFetch(`/articles/${id}`).delete();
