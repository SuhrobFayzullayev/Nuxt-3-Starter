...
export const useAdminServiceStore = defineStore("adminService", () => {
  ...
  const createOne = async (payload: zod.infer<typeof createFormSchema>) => {
    ...
    const { error } = await $authFetch("/services").post(formData);

    if (!error.value) router.push(localePath("/admin/services"));
  ...
  const fetchAll = async (page: number | string = 1) => {
    ...
    const { data, error } = await $authFetch(`/services?page=${page}`)
      .get()
      .json<{
        data: {
          data: Response[];
  ...
  const fetchOne = async (id: string | string[]) => {
    ...
    const { data, error } = await $authFetch(`/services/${id}`)
      .get()
  ...
  const updateOne = async (
    ...
    const { error } = await $authFetch(`/services/${id}`).post(formData);
  ...
  const deleteOne = async (id: number) => {
    ...
    const { error } = await $authFetch(`/services/${id}`).delete();
